...
from rag_database_generator.hirag.llms.huggingface_llm import LLM
from rag_database_generator.chunk import init_text_splitter
...
class HiRAGTextSplitter:
    def __init__(self, config: HiRAGConfig):
        ...
        self.model_config = LLM(self.hirag_config.llm_name).config()
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_config.model_id, torch_dtype=self.model_config.torch_dtype
        )
        logger.debug("Loading HiRAG LLM")
        self.model = AutoModelForCausalLM.from_pretrained(
            self.model_config.model_id, torch_dtype=self.model_config.torch_dtype, device_map=self.model_config.device
        )
        ...
        self.logits_warper = LogitsProcessorList(
            [
                ...
                TopKLogitsWarper(
                    top_k=self.model_config.top_k, min_tokens_to_keep=self.model_config.min_tokens_to_keep
                ),
                TopPLogitsWarper(
                    top_p=self.model_config.top_p, min_tokens_to_keep=self.model_config.min_tokens_to_keep
                ),
        ...
        self.logits_processor = LogitsProcessorList(
            [SequenceBiasLogitsProcessor(sequence_bias=self.model_config.sequence_bias), LogitNormalization()]
        )
        self.eos_token_id = self.tokenizer.eos_token_id
        self._running = True

        self.local_text_splitter, _ = init_text_splitter(
            self.hirag_config.chunking_method, self.hirag_config.chunk_size, self.hirag_config.chunk_overlap
        )
        self.global_text_splitter, _ = init_text_splitter(
            self.hirag_config.chunking_method,
    ...
    def _generate(self, text):
        ...
        with torch.no_grad():
            while self._running:
                ...
                attention_mask = torch.cat([attention_mask, torch.tensor([[1.0]], device=self.model_config.device)], 1)
                model_inputs.update(
                    {
                        "input_ids": input_ids,
    ...
    def global_qa_convertion(self, context: str) -> list:
        ...
        for chunk in chunk_list:
            ...
            for i, decoded_token in enumerate(self._generate(llm_input)):
                complete_answer += decoded_token
                if ":" in decoded_token:
                    text, stop = self.stop_at_n_qa(
                        complete_answer, self.hirag_config.global_understanding_QA_pair_limit
                    )
                    if stop:
                        complete_answer = text
                        self._stop()
    ...
    def generate_hirag_chunks(self, data: dict | str) -> dict:
        ...
        if isinstance(data, dict):
            logger.warning(
                "The HiRAG global understanding feature is not yet available for documents parsed into JSON file"
            )
            failed_qa_conversion = {}
            failed_data_augmentation = {}
            for key, value in tqdm(data.items(), desc="Chunking text with HiRAG"):
                text = value.pop("text")
                # Error check
                if text == "":
                    logger.warning(
                        f"You have an empty string for chunk n° {key} in {value.get('origin_file', 'unknown')} file."
                    )
                    continue
                # Add metadata
                metadata = {}
            ...
            # Perform data augmentation
            for original_qa in tqdm(combined_qa_chunk_list, desc="Chunking text with HiRAG"):
                qa_augmented_list = [original_qa]
                qa_augmented_list.extend(self.qa_data_augmentation(original_qa))
                # Error check
                if len(qa_augmented_list) == 1:
                ...
                for q, a in qa_augmented_list:
                    ...
                    # Emb_Q -> Ret_A
                    chunks_entry2 = {"chunks": [a], "complete_chunks": q.replace(";", ","), "question": q, "answer": a}
                    chunks[id] = chunks_entry2
                    id += 1
