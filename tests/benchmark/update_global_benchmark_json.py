...
from tests.benchmark.benchmark import update_json_file


def main():
    parser = argparse.ArgumentParser(
        description="Update a global metrics JSON file with a new configuration entry."
    )
    parser.add_argument(
        "new_config_file",
        type=str,
        help="Path to the JSON file containing the new, single configuration entry (e.g., Benchmark_PC_CPU_rag_danube-onnx-llm_tts_20250526_164144_910920.json).",
    )
    parser.add_argument(
        "global_metrics_file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Action to perform if configuration exists: 'compare', 'update', or 'update_if_improved'. Defaults to 'update_if_improved'.",
    )
    parser.add_argument(
        "--tolerance",
        type=float,
