# Copyright 2025 Huggingface Team
# Copyright 2026 NXP
# SPDX-License-Identifier: Apache-2.0

# Modifications made by NXP in 2026.
"""
...
DEFAULT_CHAT_TEMPLATE = ("<|im_start|>{% for message in messages %}{{message['role'] | capitalize}}"
                         ...
                         "{% for line in message['content'] %}{% if line['type'] == 'text' %}{{line['text']}}"
                         "{% elif line['type'] == 'image' %}{{ '<image>' }}{% elif line['type'] == 'video' %}"
                         "{{ '<video>' }}{% endif %}{% endfor %}<end_of_utterance>\n{% endfor %}"
...
class SmolVLMProcessor(ProcessorMixin):
    def __init__(
        ...
        **kwargs,
        ...
        image_seq_len (`int`, *optional*, defaults to 169):
            ...
            value the model used.
            It is computed as: image_seq_len = int(((image_size // patch_size) ** 2) / (scale_factor**2))
        """
        ...
        if images is not None:
            ...
            if text is not None:
                ...
                if n_images_in_images != n_images_in_text:
                    raise ValueError(
                        f"The number of images in the text {n_images_in_text} "
                        f"and images {n_images_in_images} should be the same."
                    )
            ...
            if text is not None:
                ...
                if n_videos_in_videos != n_videos_in_text:
                    raise ValueError(
                        f"The number of videos in the text {n_videos_in_text} "
                        f"and videos {n_videos_in_videos} should be the same."
                    )
    ...
    def apply_chat_template(
        ...
        **kwargs: Unpack[AllKwargsForChatTemplate],
        ...
        where each message content is a list consisting of text and optionally image or video inputs.
        One can also provide an image, video, URL or local path which will be used to form `pixel_values`
        when `return_dict=True`. If not provided, one will get only the formatted text, optionally tokenized text.
