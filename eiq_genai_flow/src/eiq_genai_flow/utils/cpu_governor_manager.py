    ...
    def save_and_set(self, new_governor="performance"):
        ...
        try:
            ...
            result = subprocess.run(
                ["sudo", "cpufreq-set", "-g", new_governor], capture_output=True, text=True, timeout=10
            )
    ...
    def restore(self):
        ...
        if self.original_governor:
            try:
                ...
                result = subprocess.run(
                    ["sudo", "cpufreq-set", "-g", self.original_governor], capture_output=True, text=True, timeout=10
                )
