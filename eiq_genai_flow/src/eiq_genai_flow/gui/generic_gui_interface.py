...
class GenericGuiInterface(threading.Thread):
    def __init__(self, callback=None, user_config=GuiConfig):
        ...
        try:
            ...
            self.mq_to_gui = posix_ipc.MessageQueue(self.user_config.egf_to_gui_queue_path, write=True, read=True)
        ...
        # Attempt to open or create the GUI to eGF QUEUE
        try:
            ...
            # If the queue already exists, open it with read and write access
            self.mq_from_gui = posix_ipc.MessageQueue(self.user_config.gui_to_egf_queue_path, write=True, read=True)
    ...
    # Command from RAG intent to Gui
    def send_intent(self, input_string):
        self._send_to_gui("INT:" + input_string)
    ...
    # Timeout to Gui
    def send_tmo(self, input_string=""):
        self._send_to_gui("TMO:" + input_string)
