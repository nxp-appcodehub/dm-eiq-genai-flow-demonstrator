...
VIT_EXE="vit"
# VIT Model Path
VIT_MODEL="-w vit/src/vit/models/VIT_Model_en.bin"
...
if [ -f "$GUI_PATH/pyproject.toml" ]; then
    ...
    if ! python3 -c "import PyQt6" 2>/dev/null; then
        echo "Missing package: PyQt6"
        DEPS_MISSING=true
    fi

    # Install only if dependencies are missing
    if [ "$DEPS_MISSING" = true ]; then
        echo "Installing/updating $GUI_NAME package and dependencies..."
        ...
        echo "$GUI_NAME dependencies are installed."
    fi
else
...
if [[ "$EGF_USE_NEUTRON" == "true" ]]; then
    echo "Neutron NPU will be used if the Platform and BSP allow it..."
    USE_NEUTRON=-n
fi
