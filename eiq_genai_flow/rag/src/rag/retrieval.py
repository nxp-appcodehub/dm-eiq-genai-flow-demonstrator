...
class Retriever:
    def __init__(
        ...
        self.chunk_list, self.embedding_list, self.reranking_embedding_list, self.metadata_list = self._split_database(
            database
        )
...
class QueryClassifier:
    def __init__(self, config: RAGConfig, retriever: Retriever, similarity_threshold: float = 0.65):
    ...
    def __call__(self, query: str, cluster_label: str | list[str] = None):
        ...
        out_of_domain_qty = sum(
            metadata.get("source", "unknown") in self.rag_config.out_of_domain_source_list for metadata in metadata_list
        )  # Garbage model or censored queries detection
