...
def main():
    ...
    def parse_args(
        rag_db_name: str = typer.Option(
            ...
            "--rag-database",
            "-d",
            help=f"RAG database file (.pkl) name in rag{os.sep}src{os.sep}data{os.sep}.",
            show_default=False
        ),
        ...
        setup_logging(
            level=logging_level, root_path=os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        )
