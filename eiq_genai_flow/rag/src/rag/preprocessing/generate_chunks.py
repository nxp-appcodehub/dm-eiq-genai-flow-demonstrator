...
def init_text_splitter(chunking_method: str, chunk_size: int, chunk_overlap: int) -> tuple:
    ...
    if chunking_method == "recursive":
        ...
        error_text = "Unknown chunking_method. Must be one of: " + str(
            [strategy.value for strategy in AvailableChunkingStrategies]
        )
        ...
        if splitter_name == "HiRAG":
            ...
            if isinstance(data, dict):
                ...
                for id, chunk in tqdm(enumerate(splited_chunks), desc=f"Chunking text with {splitter_name}"):
                    chunks[id] = {"chunks": [chunk], "source": file_name}
...
def main():
    ...
    def parse_args(
        ...
        chunking_method: AvailableChunkingStrategies = typer.Option(
            "HiRAG", "--chunking-method", "-c", help="Method used to chunk text.", show_default=True
        ),
        ...
        generate_chunks(config=config, files_to_keep=parsed_files_to_chunk, chunking_method=chunking_method)
